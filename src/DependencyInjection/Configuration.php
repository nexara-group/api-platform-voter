...
final class Configuration implements ConfigurationInterface
{
    public function getConfigTreeBuilder(): TreeBuilder
    {
        ...
        $treeBuilder->getRootNode()
            ...
            ->addDefaultsIfNotSet()
            ->children()
            ->booleanNode('auto_secure')
            ->info('Automatically secure all custom API Platform providers')
            ->defaultTrue()
            ->end()
            ->arrayNode('secure')
            ->info('List of provider service IDs to explicitly secure')
            ->scalarPrototype()->end()
            ->end()
            ->arrayNode('skip')
            ->info('List of provider service IDs to skip securing')
            ->scalarPrototype()->end()
            ->end()
            ->end()
            ->end()
            ->arrayNode('operation_mapping')
            ->addDefaultsIfNotSet()
            ->children()
            ->arrayNode('custom_operation_patterns')
            ->info('Patterns to identify custom operations (regex)')
            ->defaultValue(['!^_api_'])
            ->scalarPrototype()->end()
            ->end()
            ->enumNode('naming_convention')
            ->info('Naming convention for operation attributes')
            ->values(['snake_case', 'camelCase', 'kebab-case', 'preserve'])
            ->defaultValue('preserve')
            ->end()
            ->booleanNode('normalize_names')
            ->info('Normalize operation names to lowercase')
            ->defaultFalse()
            ->end()
            ->booleanNode('detect_by_uri')
            ->info('Detect custom operations by URI template pattern')
            ->defaultTrue()
            ->end()
            ->end()
            ->end()
